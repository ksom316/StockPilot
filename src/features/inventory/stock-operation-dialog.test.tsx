...
const product = { id: "p1", businessId: "b1", categoryId: null, categoryName: null, name: "Cable", sku: "C1", description: null, costPrice: "1", sellingPrice: "2", baseUnit: "piece", purchaseUnit: "carton", purchaseConversionQuantity: "12", currentQuantity: "10", lowStockThreshold: "2", isActive: true } satisfies Product
...
describe("StockOperationDialog", () => {
  it("shows friendly insufficient-stock feedback and preserves the form", async () => {
    ...
    await user.type(screen.getByLabelText(/quantity \(piece\)/i), "3")
    ...
    expect(screen.getByLabelText(/quantity \(piece\)/i)).toHaveValue("3")
  })
  ...
  it("prevents rapid double submission", async () => {
    ...
    render(<StockOperationDialog onClose={vi.fn()} onSubmit={onSubmit} product={product} />)
    await user.type(screen.getByLabelText(/quantity \(piece\)/i), "1")
