...
function friendlyUnit(value: string) {
  const trimmed = value.trim()
  return trimmed.toLocaleLowerCase() === "unit" ? "Not specified" : trimmed || "Not specified"
}

function meaningfulUnit(value: string) {
  const trimmed = value.trim()
  return trimmed && trimmed.toLocaleLowerCase() !== "unit" ? trimmed : ""
}

function financialUnit(value: string) {
  return meaningfulUnit(value).toLocaleLowerCase()
}

function pluralizeUnit(value: string) {
  const unit = value.trim().toLocaleLowerCase()
  if (!unit || unit === "kg") return value.trim()
  if (unit === "piece") return "pieces"
  if (unit.endsWith("s")) return value.trim()
  return `${value.trim()}s`
}
...
function PricePreview({ cost, currency, price, unit }: { cost: string; currency: string; price: string; unit: string }) {
  ...
  return <div className="mt-2 space-y-1 text-xs text-muted-foreground"><p>Cost: {formatPurchaseMoney(formatPreviewAmount(parsedCost), currency)}{unit ? ` / ${unit}` : ""}</p><p>Profit: {formatPurchaseMoney(formatPreviewAmount(profit), currency)}{unit ? ` per ${unit}` : ""}{margin !== null && <span> · Margin: {margin.toFixed(1)}%</span>}</p>{profit < 0n && <p className="text-amber-700 dark:text-amber-300" role="status">This price is below your cost of {formatPurchaseMoney(formatPreviewAmount(parsedCost), currency)}{unit ? ` per ${unit}` : ""}.</p>}</div>
}
...
export function ProductFormDialog({ product, categories: initialCategories, currency, onClose, onSubmit, onCreateCategory }: ProductFormDialogProps) {
  ...
  const unitOptions = () => <><option disabled hidden value="">Choose a unit</option>{standardUnits.map((unit) => <option key={unit} value={unit}>{unit === "unit" ? "Not specified" : unit.charAt(0).toUpperCase() + unit.slice(1)}</option>)}<option value="other">Other</option></>
  ...
  const validate = () => {
    ...
    if (!moneyPattern.test(purchaseCost.trim())) next.costPrice = `Enter what one ${friendlyUnit(purchaseUnit).toLocaleLowerCase()} costs in ${currency}.`
    ...
    if (!baseUnit.trim()) next.baseUnit = "Choose how you sell this product."
    else if (baseUnit.trim().length > 40) next.baseUnit = "Selling unit must be 40 characters or fewer."
    if (!purchaseUnit.trim()) next.purchaseUnit = "Choose how you buy this product."
    ...
    if (!quantityPattern.test(normalizedConversion) || normalizedConversion === "0" || /^0\.0*$/.test(normalizedConversion)) next.purchaseConversionQuantity = `Enter the quantity for this purchase-to-selling unit relationship.`
    const derivedCost = calculateBaseUnitCost(purchaseCost.trim(), normalizedConversion)
    if (moneyPattern.test(purchaseCost.trim()) && !derivedCost) next.costPrice = `This purchase cost cannot be converted safely to a ${friendlyUnit(baseUnit)} cost.`
  ...
  return (
    <DialogShell description={product ? "Update catalog details without changing stock." : "Create a zero-stock catalog product. Stock can be added later."} onClose={onClose} title={product ? "Edit product" : "Add product"} wide>
      <form className="space-y-5" noValidate onSubmit={handleSubmit}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2"><label className="block text-sm font-medium" htmlFor="purchase-unit">I buy this product by</label><select className="h-11 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" disabled={isSubmitting} id="purchase-unit" onChange={(event) => selectUnit(event.target.value, setPurchaseUnit)} value={purchaseUnitIsCustom ? "other" : purchaseUnit.trim().toLocaleLowerCase()}>{unitOptions()}</select>{purchaseUnitIsCustom && <FormField disabled={isSubmitting} error={errors.purchaseUnit} id="custom-purchase-unit" label="Unit name" maxLength={40} onChange={(event) => setPurchaseUnit(event.target.value)} placeholder="e.g. tray" value={purchaseUnit} />}{errors.purchaseUnit && !purchaseUnitIsCustom && <p className="text-sm text-destructive">{errors.purchaseUnit}</p>}</div>
          <FormField disabled={isSubmitting} error={errors.costPrice} id="purchase-cost" inputMode="decimal" label={`${financialUnit(purchaseUnit) ? `Cost per ${financialUnit(purchaseUnit)}` : "Cost"} (${currency})`} onChange={(event) => setPurchaseCost(event.target.value)} value={purchaseCost} />
        </div>
        {purchaseUnit.trim().toLocaleLowerCase() !== baseUnit.trim().toLocaleLowerCase() && (meaningfulUnit(baseUnit) && meaningfulUnit(purchaseUnit) ? <div className="space-y-2"><div className="grid grid-cols-[1fr_auto] items-end gap-3"><FormField disabled={isSubmitting} error={errors.purchaseConversionQuantity} id="purchase-conversion-quantity" inputMode="decimal" label={`One ${friendlyUnit(purchaseUnit)} contains`} onChange={(event) => setPurchaseConversionQuantity(event.target.value)} value={purchaseConversionQuantity} /><p className="pb-3 text-sm text-muted-foreground">{pluralizeUnit(friendlyUnit(baseUnit))}</p></div><p className="-mt-1 text-sm text-muted-foreground">One {friendlyUnit(purchaseUnit)} contains {purchaseConversionQuantity || "?"} {pluralizeUnit(friendlyUnit(baseUnit))}</p></div> : <p className="rounded-md bg-muted/40 p-3 text-sm text-muted-foreground">Choose how you sell this product to set up the conversion.</p>)}
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2"><label className="block text-sm font-medium" htmlFor="base-unit">I sell this product by</label><select className="h-11 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" disabled={isSubmitting} id="base-unit" onChange={(event) => selectUnit(event.target.value, setBaseUnit)} value={baseUnitIsCustom ? "other" : baseUnit.trim().toLocaleLowerCase()}>{unitOptions()}</select>{baseUnitIsCustom && <FormField disabled={isSubmitting} error={errors.baseUnit} id="custom-base-unit" label="Unit name" maxLength={40} onChange={(event) => setBaseUnit(event.target.value)} placeholder="e.g. tray" value={baseUnit} />}{errors.baseUnit && !baseUnitIsCustom && <p className="text-sm text-destructive">{errors.baseUnit}</p>}</div>
          <FormField disabled={isSubmitting} error={errors.sellingPrice} id="selling-price" inputMode="decimal" label={`${financialUnit(baseUnit) ? `Selling price per ${financialUnit(baseUnit)}` : "Selling price"} (${currency})`} onChange={(event) => setSellingPrice(event.target.value)} value={sellingPrice} />
        </div>
        {baseCostPreview && <div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-sm"><p>Your cost: <strong>{formatPurchaseMoney(baseCostPreview, currency)}{financialUnit(baseUnit) ? ` per ${financialUnit(baseUnit)}` : ""}</strong></p><PricePreview cost={baseCostPreview} currency={currency} price={sellingPrice} unit={financialUnit(baseUnit)} /></div>}
        <section aria-labelledby="additional-selling-units-heading" className="space-y-3 rounded-lg border border-border p-4">
          <div><h3 className="font-medium" id="additional-selling-units-heading">Sell this product another way?</h3><p className="mt-1 text-sm text-muted-foreground">You can sell the same stock in another unit at a different price.</p></div>
          {sellingUnits.map((sellingUnit, index) => <div className="rounded-md border border-border bg-muted/20 p-3" key={index}><div className="grid gap-4 sm:grid-cols-3"><FormField disabled={isSubmitting} id={`selling-unit-${index}`} label="I sell by" maxLength={40} onChange={(event) => setSellingUnits((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, unit: event.target.value } : item))} placeholder="carton" value={sellingUnit.unit} /><FormField disabled={isSubmitting} id={`selling-unit-conversion-${index}`} inputMode="decimal" label={`One ${sellingUnit.unit.trim() || "unit"} contains`} onChange={(event) => setSellingUnits((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, conversionQuantity: event.target.value } : item))} value={sellingUnit.conversionQuantity} /><FormField disabled={isSubmitting} id={`selling-unit-price-${index}`} inputMode="decimal" label={`Price per ${sellingUnit.unit.trim() || "unit"} (${currency})`} onChange={(event) => setSellingUnits((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, sellingPrice: event.target.value } : item))} value={sellingUnit.sellingPrice} /></div><p className="mt-2 text-sm text-muted-foreground">{sellingUnit.unit.trim() && baseUnit.trim() ? `One ${sellingUnit.unit.trim()} contains ${sellingUnit.conversionQuantity || "?"} ${pluralizeUnit(friendlyUnit(baseUnit))}` : "Choose another selling option."}</p>{baseCostPreview && <PricePreview cost={calculatePurchaseLineTotal(sellingUnit.conversionQuantity, baseCostPreview) ?? baseCostPreview} currency={currency} price={sellingUnit.sellingPrice} unit={sellingUnit.unit.trim() || "selling unit"} />}<Button disabled={isSubmitting} onClick={() => setSellingUnits((current) => current.filter((_, itemIndex) => itemIndex !== index))} size="sm" type="button" variant="outline">Remove</Button></div>)}
          <Button disabled={isSubmitting} onClick={() => setSellingUnits((current) => [...current, { unit: "", conversionQuantity: "1", sellingPrice: sellingPrice }])} type="button" variant="outline">+ Add another selling option</Button>
          {errors.sellingUnits && <p className="text-sm text-destructive" role="alert">{errors.sellingUnits}</p>}
        </section>
        <FormField disabled={isSubmitting} error={errors.lowStockThreshold} id="low-stock-threshold" inputMode="decimal" label={`Low-stock threshold (${friendlyUnit(baseUnit)})`} onChange={(event) => setLowStockThreshold(event.target.value)} value={lowStockThreshold} />
        <div className="space-y-2">
