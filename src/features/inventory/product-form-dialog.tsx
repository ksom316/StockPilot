...
function friendlyUnit(value: string) {
  ...
  return trimmed.toLocaleLowerCase() === "unit" ? "generic unit" : trimmed || "unit"
}
...
export function ProductFormDialog({ product, categories: initialCategories, currency, onClose, onSubmit, onCreateCategory }: ProductFormDialogProps) {
  ...
  const unitOptions = () => <><option value="">Choose a unit</option>{standardUnits.map((unit) => <option key={unit} value={unit}>{unit === "unit" ? "Generic unit" : unit}</option>)}<option value="other">Other</option></>
  ...
  const validate = () => {
    ...
    if (!moneyPattern.test(purchaseCost.trim())) next.costPrice = `Enter what one ${friendlyUnit(purchaseUnit)} costs in ${currency}.`
    ...
    if (!quantityPattern.test(normalizedConversion) || normalizedConversion === "0" || /^0\.0*$/.test(normalizedConversion)) next.purchaseConversionQuantity = `Enter how many ${friendlyUnit(baseUnit)} are in one ${friendlyUnit(purchaseUnit)}.`
  ...
  return (
    <DialogShell description={product ? "Update catalog details without changing stock." : "Create a zero-stock catalog product. Stock can be added later."} onClose={onClose} title={product ? "Edit product" : "Add product"} wide>
      <form className="space-y-5" noValidate onSubmit={handleSubmit}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2"><label className="block text-sm font-medium" htmlFor="purchase-unit">How do you buy this product?</label><select className="h-11 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" disabled={isSubmitting} id="purchase-unit" onChange={(event) => selectUnit(event.target.value, setPurchaseUnit)} value={purchaseUnitIsCustom ? "other" : purchaseUnit.trim().toLocaleLowerCase()}>{unitOptions()}</select>{purchaseUnitIsCustom && <FormField disabled={isSubmitting} error={errors.purchaseUnit} id="custom-purchase-unit" label="Custom purchase unit" maxLength={40} onChange={(event) => setPurchaseUnit(event.target.value)} placeholder="e.g. bundle" value={purchaseUnit} />}{errors.purchaseUnit && !purchaseUnitIsCustom && <p className="text-sm text-destructive">{errors.purchaseUnit}</p>}</div>
        ...
        {purchaseUnit.trim().toLocaleLowerCase() !== baseUnit.trim().toLocaleLowerCase() && <><FormField disabled={isSubmitting} error={errors.purchaseConversionQuantity} id="purchase-conversion-quantity" inputMode="decimal" label={`How many ${friendlyUnit(baseUnit)} are in one ${friendlyUnit(purchaseUnit)}?`} onChange={(event) => setPurchaseConversionQuantity(event.target.value)} value={purchaseConversionQuantity} />{baseUnit.trim() && purchaseUnit.trim() && <p className="-mt-3 text-sm text-muted-foreground">1 {friendlyUnit(purchaseUnit)} contains {purchaseConversionQuantity || "?"} {friendlyUnit(baseUnit)}</p>}</>}
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2"><label className="block text-sm font-medium" htmlFor="base-unit">How do you sell this product?</label><select className="h-11 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" disabled={isSubmitting} id="base-unit" onChange={(event) => selectUnit(event.target.value, setBaseUnit)} value={baseUnitIsCustom ? "other" : baseUnit.trim().toLocaleLowerCase()}>{unitOptions()}</select>{baseUnitIsCustom && <FormField disabled={isSubmitting} error={errors.baseUnit} id="custom-base-unit" label="Custom selling unit" maxLength={40} onChange={(event) => setBaseUnit(event.target.value)} placeholder="e.g. tray" value={baseUnit} />}{errors.baseUnit && !baseUnitIsCustom && <p className="text-sm text-destructive">{errors.baseUnit}</p>}</div>
          <FormField disabled={isSubmitting} error={errors.sellingPrice} id="selling-price" inputMode="decimal" label={`Selling price per ${friendlyUnit(baseUnit)} (${currency})`} onChange={(event) => setSellingPrice(event.target.value)} value={sellingPrice} />
        </div>
        {baseCostPreview && <div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-sm"><p>Your cost: <strong>{formatPurchaseMoney(baseCostPreview, currency)} per {friendlyUnit(baseUnit)}</strong></p><PricePreview cost={baseCostPreview} currency={currency} price={sellingPrice} unit={friendlyUnit(baseUnit)} /></div>}
