    ...
    <DialogShell description={product ? "Update catalog details without changing stock." : "Create a zero-stock catalog product. Stock can be added later."} onClose={onClose} title={product ? "Edit product" : "Add product"} wide>
      <form className="space-y-5" noValidate onSubmit={handleSubmit}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <FormField disabled={isSubmitting} error={errors.costPrice} id="purchase-cost" inputMode="decimal" label={`Cost per ${friendlyUnit(purchaseUnit)} (${currency})`} onChange={(event) => setPurchaseCost(event.target.value)} value={purchaseCost} />
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <FormField disabled={isSubmitting} error={errors.sellingPrice} id="selling-price" inputMode="decimal" label={`Selling price per ${friendlyUnit(baseUnit).toLocaleLowerCase()} (${currency})`} onChange={(event) => setSellingPrice(event.target.value)} value={sellingPrice} />
        </div>
        {baseCostPreview && <div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-sm"><p>Your cost: <strong>{formatPurchaseMoney(baseCostPreview, currency)} per {friendlyUnit(baseUnit).toLocaleLowerCase()}</strong></p><PricePreview cost={baseCostPreview} currency={currency} price={sellingPrice} unit={friendlyUnit(baseUnit).toLocaleLowerCase()} /></div>}
