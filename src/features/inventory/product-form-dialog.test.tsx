...
describe("ProductFormDialog", () => {
  it("validates required product details and numeric precision", async () => {
    ...
    await user.clear(within(dialog).getByLabelText(/cost per generic unit/i))
    await user.type(within(dialog).getByLabelText(/cost per generic unit/i), "1.12345")
    ...
    expect(within(dialog).getByText(/enter what one generic unit costs/i)).toBeInTheDocument()
  ...
  it("captures purchase-to-selling-unit conversion and rejects zero", async () => {
    ...
    await user.selectOptions(within(dialog).getByLabelText(/how do you sell this product/i), "kg")
    await user.selectOptions(within(dialog).getByLabelText(/how do you buy this product/i), "carton")
  ...
  it("shows independent bulk pricing and a below-cost warning", async () => {
    ...
    await user.type(within(dialog).getByLabelText(/product name/i), "Chicken Wings")
    await user.clear(within(dialog).getByLabelText(/cost per generic unit/i))
    await user.type(within(dialog).getByLabelText(/cost per generic unit/i), "30")
    await user.selectOptions(within(dialog).getByLabelText(/how do you sell this product/i), "kg")
    await user.clear(within(dialog).getByLabelText(/selling price per kg/i))
  ...
  it("shows conversion guidance and a purchase-unit cost label for different units", async () => {
    ...
    expect(within(dialog).getByLabelText(/how many kg are in one carton/i)).toBeInTheDocument()
    expect(within(dialog).getByText(/1 carton contains/i)).toBeInTheDocument()
  })
  ...
  it("hides conversion controls when selling and purchase units match", async () => {
    ...
    render(<ProductFormDialog categories={[]} currency="GHS" onClose={vi.fn()} onSubmit={vi.fn()} />)
    const dialog = screen.getByRole("dialog")
    await user.selectOptions(within(dialog).getByLabelText(/how do you sell this product/i), "piece")
    await user.selectOptions(within(dialog).getByLabelText(/how do you buy this product/i), "piece")
    expect(within(dialog).queryByLabelText(/how many piece are in one piece/i)).not.toBeInTheDocument()
  })
  ...
  it("supports piece selling units purchased by the carton", async () => {
    ...
    await user.selectOptions(within(dialog).getByLabelText(/how do you sell this product/i), "piece")
    await user.selectOptions(within(dialog).getByLabelText(/how do you buy this product/i), "carton")
    expect(within(dialog).getByLabelText(/how many piece are in one carton/i)).toBeInTheDocument()
    expect(within(dialog).getByLabelText(/cost per carton \(GHS\)/i)).toBeInTheDocument()
  })
  ...
  it("normalizes custom units selected through Other", async () => {
    ...
    await user.selectOptions(within(dialog).getByLabelText(/how do you sell this product/i), "other")
    await user.type(within(dialog).getByLabelText(/custom selling unit/i), "  Tray  ")
    await user.selectOptions(within(dialog).getByLabelText(/how do you buy this product/i), "other")
    await user.type(within(dialog).getByLabelText(/custom purchase unit/i), "  Bundle  ")
    await user.clear(within(dialog).getByLabelText(/how many/i))
    await user.type(within(dialog).getByLabelText(/how many/i), "4")
    await user.click(within(dialog).getByRole("button", { name: /add product/i }))
  ...
  it("keeps an existing non-standard unit on the Other path while editing", () => {
    ...
    const dialog = screen.getByRole("dialog")
    expect(within(dialog).getByLabelText(/how do you sell this product/i)).toHaveValue("other")
    expect(within(dialog).getByLabelText(/custom selling unit/i)).toHaveValue("Tray")
    expect(within(dialog).getByLabelText(/how do you buy this product/i)).toHaveValue("other")
    expect(within(dialog).getByLabelText(/custom purchase unit/i)).toHaveValue("Bundle")
  })
