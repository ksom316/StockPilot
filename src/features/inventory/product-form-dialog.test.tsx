...
describe("ProductFormDialog", () => {
  ...
  it("captures purchase-to-selling-unit conversion and rejects zero", async () => {
    ...
    await user.selectOptions(within(dialog).getByLabelText(/^selling unit$/i), "kg")
    await user.selectOptions(within(dialog).getByLabelText(/^purchase unit$/i), "carton")
  ...
  it("hides conversion controls when selling and purchase units match", async () => {
    ...
    render(<ProductFormDialog categories={[]} currency="GHS" onClose={vi.fn()} onSubmit={vi.fn()} />)
    const dialog = screen.getByRole("dialog")
    await user.selectOptions(within(dialog).getByLabelText(/^selling unit$/i), "piece")
    await user.selectOptions(within(dialog).getByLabelText(/^purchase unit$/i), "piece")
  ...
  it("normalizes custom units selected through Other", async () => {
    ...
    await user.selectOptions(within(dialog).getByLabelText(/^selling unit$/i), "other")
    await user.type(within(dialog).getByLabelText(/custom selling unit/i), "  Tray  ")
    await user.selectOptions(within(dialog).getByLabelText(/^purchase unit$/i), "other")
  ...
  it("keeps an existing non-standard unit on the Other path while editing", () => {
    ...
    const dialog = screen.getByRole("dialog")
    expect(within(dialog).getByLabelText(/^selling unit$/i)).toHaveValue("other")
    expect(within(dialog).getByLabelText(/custom selling unit/i)).toHaveValue("Tray")
    expect(within(dialog).getByLabelText(/^purchase unit$/i)).toHaveValue("other")
