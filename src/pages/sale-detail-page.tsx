    ...
    <section className="space-y-6">
      ...
      {!sale.isLoading && !sale.isError && sale.data && business && (
        <>
          ...
          <dl className="grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-2"><div><dt className="text-sm text-muted-foreground">Recorded by</dt><dd className="mt-1 font-medium">Team member</dd></div><div><dt className="text-sm text-muted-foreground">Items</dt><dd className="mt-1 font-medium">{sale.data.items.length}</dd></div>{sale.data.notes && <div className="sm:col-span-2"><dt className="text-sm text-muted-foreground">Note</dt><dd className="mt-1 whitespace-pre-wrap break-words">{sale.data.notes}</dd></div>}</dl>
