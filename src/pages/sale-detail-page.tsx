...
export function SaleDetailPage() {
  ...
  return (
    <section className="space-y-6">
      <SalesSectionNav />
      {sale.isLoading && <div className="rounded-xl border border-border bg-card p-8 text-center text-muted-foreground" role="status">Loading sale details…</div>}
      ...
      {!sale.isLoading && !sale.isError && sale.data && business && (
        <>
          <header className="flex flex-wrap items-end justify-between gap-4">
            <div><Button asChild className="mb-4" size="sm" variant="outline"><Link to="/sales/history"><ArrowLeft aria-hidden="true" className="mr-2 size-4" />Sales history</Link></Button><p className="text-sm font-medium text-primary">Sale details</p><h1 className="mt-1 break-all text-3xl font-semibold tracking-tight">{sale.data.saleReference}</h1><p className="mt-2 text-muted-foreground">{formatSaleDate(sale.data.soldAt)}</p></div>
            <div className="min-w-40 rounded-xl border border-border bg-card p-4"><p className="text-sm text-muted-foreground">Total</p><p className="mt-1 text-2xl font-semibold tabular-nums">{formatSaleMoney(sale.data.total, business.currency)}</p></div>
          </header>
          <dl className="grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-2"><div><dt className="text-sm text-muted-foreground">Customer</dt><dd className="mt-1 font-medium">{sale.data.customerNameSnapshot ?? "Walk-in"}</dd></div><div><dt className="text-sm text-muted-foreground">Recorded by</dt><dd className="mt-1 font-medium">Team member</dd></div><div><dt className="text-sm text-muted-foreground">Items</dt><dd className="mt-1 font-medium">{sale.data.items.length}</dd></div>{sale.data.notes && <div className="sm:col-span-2"><dt className="text-sm text-muted-foreground">Note</dt><dd className="mt-1 whitespace-pre-wrap break-words">{sale.data.notes}</dd></div>}</dl>
          <section aria-labelledby="sale-items-heading" className="overflow-hidden rounded-xl border border-border bg-card">
            <div className="border-b border-border px-4 py-4"><h2 className="font-semibold" id="sale-items-heading">Items sold</h2></div>
            ...
            <div className="flex items-center justify-between border-t border-border px-4 py-4"><span className="text-sm text-muted-foreground">Subtotal / total</span><span className="font-semibold tabular-nums">{formatSaleMoney(sale.data.total, business.currency)}</span></div>
          </section>
        </>
      )}
    </section>
