...
function labelChannel(value: string) { return ({ walk_in: "Walk-in", pickup: "Pickup", delivery: "Delivery", other: "Other" } as Record<string, string>)[value] ?? "—" }
function labelPayment(value: string) { return ({ cash: "Cash", mobile_money: "Mobile Money", card: "Card", bank_transfer: "Bank Transfer" } as Record<string, string>)[value] ?? "—" }
    ...
    <section className="space-y-6">
      ...
      {!sale.isLoading && !sale.isError && sale.data && business && (
        <>
          ...
          <dl className="grid gap-3 rounded-lg border border-border bg-card p-4 sm:grid-cols-2"><div><dt className="text-sm text-muted-foreground">Customer</dt><dd className="mt-1 font-medium">{sale.data.customerNameSnapshot ?? "—"}</dd></div><div><dt className="text-sm text-muted-foreground">Sales channel</dt><dd className="mt-1 font-medium">{labelChannel(sale.data.salesChannel ?? "walk_in")}</dd></div><div><dt className="text-sm text-muted-foreground">Payment method</dt><dd className="mt-1 font-medium">{labelPayment(sale.data.paymentMethod ?? "cash")}</dd></div><div><dt className="text-sm text-muted-foreground">Recorded by</dt><dd className="mt-1 font-medium">{labelRecorder(sale.data.recordedByName, sale.data.recordedByRole)}</dd></div><div><dt className="text-sm text-muted-foreground">Items</dt><dd className="mt-1 font-medium">{sale.data.items.length}</dd></div>{sale.data.notes && <div className="sm:col-span-2"><dt className="text-sm text-muted-foreground">Note</dt><dd className="mt-1 whitespace-pre-wrap break-words">{sale.data.notes}</dd></div>}</dl>
