...
export function AnalystPage() {
  ...
  return <section className="space-y-6">
    <header><p className="text-sm font-medium text-primary">AI Analyst</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Ask about your business</h1><p className="mt-2 max-w-2xl text-muted-foreground">Get grounded answers based on recorded StockPilot business data. AI Analyst is a single-turn summary tool, not an accounting system or recommendation engine.</p></header>

    <form className="rounded-xl border bg-card p-5 shadow-sm sm:p-6" onSubmit={(event) => { void submit(event).catch(() => undefined) }}>
      <div className="grid gap-4 sm:grid-cols-[minmax(0,0.35fr)_minmax(0,1fr)] sm:items-end">
    ...
    <section aria-labelledby="starter-questions-title" className="rounded-xl border border-dashed p-5"><h2 className="font-semibold" id="starter-questions-title">Try a grounded question</h2><div className="mt-3 flex flex-wrap gap-2">{suggestions.map((suggestion) => <button className="rounded-full border border-border bg-card px-3 py-2 text-left text-sm text-muted-foreground hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30" key={suggestion} onClick={() => setQuestion(suggestion)} type="button">{suggestion}</button>)}</div></section>
