...
export function FinanceExpensesPage() {
  ...
  return <section className="space-y-6">
    ...
    {formOpen && <form aria-labelledby="expense-form-title" className="grid gap-4 rounded-lg border border-border bg-card p-4 sm:grid-cols-2" noValidate onSubmit={(event) => void submit(event)}>
    ...
    {expensesQuery.isLoading && <LoadingState className="rounded-lg border border-border bg-card p-8 text-center" label="Loading expenses…" />}
    {expensesQuery.isError && <ErrorState onRetry={() => void expensesQuery.refetch()} title="Expenses unavailable">Check that Finance is enabled and try again.</ErrorState>}
    {!expensesQuery.isLoading && !expensesQuery.isError && filtered.length === 0 && <EmptyState description={expensesQuery.data?.length ? "Adjust or clear your filters." : "Operating expenses you record will appear here. Stock purchases remain in Purchasing."} title={expensesQuery.data?.length ? "No matching expenses" : "No expenses yet"} />}
    {filtered.length > 0 && business && <>
      ...
      <TableContainer className="hidden md:block">
        <Table><caption className="sr-only">Business expenses</caption>
          <TableHead><tr>{["Date", "Description", "Category", "Status", "Amount", "Actions"].map((label) => <Th align={label === "Amount" ? "right" : "left"} key={label}>{label}</Th>)}</tr></TableHead>
          <TableBody>{filtered.map((expense) => <TableRow className={expense.voided ? "text-muted-foreground" : "hover:bg-muted/30"} key={expense.id}>
            <Td className="whitespace-nowrap">{expense.expenseDate}</Td>
            <Td className="max-w-xs"><p className="break-words font-medium">{expense.description}</p></Td>
            <Td>{expense.categoryName}</Td>
            <Td><StatusBadge voided={expense.voided} /></Td>
            <Td align="right" className="whitespace-nowrap font-medium tabular-nums">{formatExpenseMoney(expense.amount, business.currency)}</Td>
            <Td><ExpenseActions expense={expense} onDetails={() => setSelected(expense)} onEdit={() => beginForm(expense)} onVoid={() => { setVoidTarget(expense); setError(""); setStatus("") }} /></Td>
          </TableRow>)}</TableBody>
        </Table>
      </TableContainer>
      <ul aria-label="Business expenses" className="space-y-3 md:hidden">{filtered.map((expense) => <li className={`rounded-lg border border-border bg-card p-4 ${expense.voided ? "opacity-75" : ""}`} key={expense.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-semibold">{expense.description}</h2><p className="mt-1 text-sm text-muted-foreground">{expense.expenseDate} · {expense.categoryName}</p></div><strong className="shrink-0 text-right tabular-nums">{formatExpenseMoney(expense.amount, business.currency)}</strong></div><div className="mt-3 flex items-center justify-between gap-2"><StatusBadge voided={expense.voided} /><ExpenseActions expense={expense} onDetails={() => setSelected(expense)} onEdit={() => beginForm(expense)} onVoid={() => { setVoidTarget(expense); setError(""); setStatus("") }} /></div></li>)}</ul>
    </>}

    <section aria-labelledby="categories-title" className="space-y-4 rounded-lg border border-border bg-card p-4"><div><h2 className="font-semibold" id="categories-title">Expense categories</h2><p className="mt-1 text-sm text-muted-foreground">Default categories are fixed. Custom categories can be renamed or deactivated; history is preserved.</p></div>
...
function StatusBadge({ voided }: { voided: boolean }) { return <Badge variant={voided ? "neutral" : "success"}>{voided ? "Voided" : "Active"}</Badge> }
