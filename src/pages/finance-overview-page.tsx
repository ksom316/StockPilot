...
export function FinanceOverviewPage() {
  ...
  return <section className="space-y-6">
    <FinanceSectionNav />
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-primary">Finance</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Overview</h1><p className="mt-2 text-muted-foreground">Recorded sales, operating expenses, and estimated profitability for your business.</p></div><Button asChild variant="outline"><Link to="/finance/expenses">Manage Expenses</Link></Button></header>

    <section aria-label="Financial period" className="grid gap-3 rounded-xl border bg-card p-4 sm:grid-cols-3 sm:items-end">
    ...
    {summary.data && business && <>
      {summary.data.saleCount === 0 && summary.data.operatingExpenses === "0.0000" && (summary.data.purchaseReceipts === null || summary.data.purchaseReceipts === "0.0000") && <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No sales, expenses, or stock receipts were recorded in this period.</p>}
...
function Metric({ label, value, help }: { label: string; value: string; help: string }) {
  return <article className="min-w-0 rounded-xl border bg-card p-5"><h2 className="text-sm font-medium text-muted-foreground">{label}</h2><p className="mt-3 break-all text-2xl font-semibold tabular-nums sm:text-3xl">{value}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{help}</p></article>
}
