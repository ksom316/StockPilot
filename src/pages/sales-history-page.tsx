...
function SaleRow({ sale, currency }: { sale: SaleSummary; currency: string }) {
  return (
    <tr className="border-b border-border last:border-0">
      <td className="px-4 py-3"><Link className="font-medium text-primary hover:underline focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30" to={`/sales/${sale.id}`}>{sale.saleReference}</Link><div className="mt-1 max-w-xs break-words text-xs text-muted-foreground">{sale.notes ? `Note: ${sale.notes}` : "No note"}</div></td>
...
export function SalesHistoryPage() {
  ...
  const filteredSales = useMemo(() => {
    ...
    return (history.data ?? []).filter((sale) => {
      ...
      const matchesSearch = !normalizedSearch || sale.saleReference.toLocaleLowerCase().includes(normalizedSearch) || sale.items.some((item) => item.productName.toLocaleLowerCase().includes(normalizedSearch) || item.productSku.toLocaleLowerCase().includes(normalizedSearch))
  ...
  return (
    <section className="space-y-6">
      ...
      <section aria-label="Filter sales" className="grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-[minmax(0,1fr)_repeat(2,minmax(150px,0.35fr))_auto] sm:items-end">
        <label className="space-y-1.5"><span className="text-sm font-medium">Search sales</span><input aria-label="Search sales" autoComplete="off" className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" onChange={(event) => updateParam("q", event.target.value)} placeholder="Reference, product or SKU" type="search" value={search} /></label>
