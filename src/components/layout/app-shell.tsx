        ...
        {session && business && (
          <nav aria-label="Workspace navigation" className="border-t border-border">
            <div className="mx-auto flex max-w-6xl items-center gap-5 overflow-x-auto px-4 py-3 text-sm sm:px-6">
              ...
              {enabledModules.filter((module) => module !== "purchasing" || ["owner", "manager", "employee"].includes(role ?? "")).filter((module) => module !== "smart_insights" || ["owner", "manager", "employee"].includes(role ?? "")).map((module) => (
                module === "sales" ? (
