...
begin
  ...
  insert into public.businesses (name, business_type, owner_user_id, icon_id) values (btrim(p_name), nullif(btrim(p_business_type), ''), v_user_id, p_icon_id) returning id into v_business_id;
...
revoke all on function public.create_business_onboarding(text, text, public.optional_module[], text) from public, anon;
grant execute on function public.create_business_onboarding(text, text, public.optional_module[], text) to authenticated;
