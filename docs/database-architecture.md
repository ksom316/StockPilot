...
The observation window is the 30 most recently completed dates in the business's validated IANA timezone: `[local midnight 30 dates before today, today's local midnight)`. The current partial local date is excluded, and local boundaries are converted to timestamps in PostgreSQL so client/session timezones cannot shift them. Product creation and the Sales module row's last `updated_at` transition conservatively establish the first provably observable full local date. Because no richer module-state history exists, products without 30 proven dates return `INSUFFICIENT_HISTORY`; the RPC never fabricates historical Sales coverage.
