...
The initial context excludes Customers entirely, even when enabled. It also excludes customer identifiers and contact data, supplier contacts/notes, raw purchase lines/costs, expense descriptions/notes, product descriptions, transaction notes, and other arbitrary user-authored prose. Product names and SKUs are the minimum untrusted database text and must be treated as data by the server prompt boundary. Phase 10 explains authorized deterministic facts only; recommendations, opportunity detection, forecasting, and external trends remain out of scope. Opportunity synthesis remains a Phase 11 concern.

## AI Analyst provider and usage boundary

The `ai-analyst` Edge Function is the only Phase 10B provider boundary. It verifies the user with Supabase Auth and invokes `get_ai_analyst_context` with the same caller JWT. It never uses service-role privileges to retrieve business facts. A separate service client is confined to service-role-only quota reservation/completion RPCs, which independently check the verified user ID against active owner/manager membership and module state. OpenRouter configuration remains server-only, the selected model is deployment configuration rather than browser input, and automated tests use an injected fake provider.

`ai_analyst_usage` is a content-free operational ledger with no direct authenticated table grants. Its reservation RPC requires an active owner/manager and enabled AI Analyst module, then uses transaction-scoped advisory locks to enforce 10 calls per user in a rolling hour and 50 per business-local day without an ordinary simultaneous-request bypass. A reserved call remains counted if the provider fails. Only normalized status/error, provider/model, tokens, and latency may be added; prompts, questions, context, answers, and provider payloads are prohibited.

The provider returns prose plus evidence IDs, never authoritative values. The Edge Function filters those IDs against a request-specific registry and hydrates exact values from schema-version-1 context. Unknown IDs are removed deterministically. Context labels and the question are untrusted data, there are no provider tools, and Customers remains absent. There is no conversation persistence. The detailed trust, response, privacy, error, CORS, and local-smoke contracts are in `docs/ai-analyst-provider.md`.
